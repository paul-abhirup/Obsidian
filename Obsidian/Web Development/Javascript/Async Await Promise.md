...
Parallel Excecution with promise.all()  --->
```js
...
3. You can only call `await` inside a function if that function is `async`
4. You cant have a `top level await`
5. its a syntactical sugar that lets you write async code in a sync style
6. Build on top of Promises and allows you to avoid chaining `.then()` and `.catch()` methods while still working with asynchronous operations.
